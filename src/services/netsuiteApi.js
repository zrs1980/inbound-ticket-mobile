...
export async function getTickets({ query = '', limit = 50, offset = 0 } = {}) {
  ...
  return request('GET', '/query/v1/suiteql', { q, limit });
}

export async function getTicket(id) {
  return request('GET', `/customrecord_looperp_inb_tckt/${id}`, { expandSubResources: 'true' });
}

export async function createTicket(data) {
  return request('POST', '/customrecord_looperp_inb_tckt', {}, data);
}

export async function updateTicket(id, data) {
  return request('PATCH', `/customrecord_looperp_inb_tckt/${id}`, {}, data);
}
...
export async function createTicketLine(data) {
  return request('POST', '/customrecord_looperp_inb_tckt_ln', {}, data);
}

export async function updateTicketLine(id, data) {
  return request('PATCH', `/customrecord_looperp_inb_tckt_ln/${id}`, {}, data);
}

export async function deleteTicketLine(id) {
  return request('DELETE', `/customrecord_looperp_inb_tckt_ln/${id}`);
}
